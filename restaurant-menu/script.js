 ...
 * make over to select button when they are available
 */
...
// Given function to call the DOMContentLoaded
function displayMenuItems(menuItems) {
  let displayMenu = menuItems.map(function (item) {
    return `
        <article class="menu-item">
          <img src=${item.img} alt=${"item-" + item.id} class="photo">
          <div class="item-info">
            <header>
              <h4>${item.title}</h4>
  ...
  displayMenu = displayMenu.join(""); // This is important otherwise display to comma
...
function displayMenuButtons() {
  ...
  filterBtns.forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      // console.log(e.currentTarget.dataset.id);   // dataset used to data- in html
